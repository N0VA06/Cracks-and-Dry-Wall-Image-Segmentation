...
def mode_train(args, device):
    ...
    label = "cracks + drywall (combined)" if dataset_name == "all" else dataset_name
    print(f"\n[Train] Dataset: {label}  |  Model: {args.model}")

    # ── Tokenizer + text encoder (always on — text conditioning is the goal) ──
    tokenizer, text_encoder = build_tokenizer_and_encoder(device)

    # ── DataLoaders ───────────────────────────────────────────────────
    # SAM ViT-B upsamples to 1024×1024 internally → use smaller batch to avoid OOM
    effective_batch = SAM_BATCH_SIZE if args.model == "sam" else BATCH_SIZE
    ...
    if dataset_name == "all":
        ...
        train_loader, val_loader, _ = build_combined_loaders(
            tokenizer, batch_size=effective_batch
        )
...
def main():
    ...
    if args.mode == "split":
        ...
        # ALWAYS call mode_train once.
        # When --dataset all, mode_train internally uses build_combined_loaders()
        # which mixes both datasets in the same DataLoader via WeightedRandomSampler.
        # The model sees "segment crack" and "segment taping area" in the same
        # training run — that is what trains the FiLM text conditioning.
        # Training twice (once per dataset) produces two single-task models and
        # overwrites the checkpoint, which is wrong.
